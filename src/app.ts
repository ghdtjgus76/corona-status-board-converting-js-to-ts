...
function setDeathsList(data: any) {
  const sorted = data.sort(
    (a: any, b: any): any => getUnixTimestamp(b.Date) - getUnixTimestamp(a.Date),
  );
...
function setTotalConfirmedNumber(data: any) {
  confirmedTotal.innerText = data.Countries.reduce(
    (total: any, current: any): any => (total += current.TotalConfirmed),
    0,
  );
}
...
function setTotalDeathsByWorld(data: any) {
  deathsTotal.innerText = data.Countries.reduce(
    (total: any, current: any): any => (total += current.TotalDeaths),
    0,
  );
}
...
function setTotalRecoveredByWorld(data: any) {
  recoveredTotal.innerText = data.Countries.reduce(
    (total: any, current: any): any => (total += current.TotalRecovered),
    0,
  );
}
...
function setCountryRanksByConfirmedCases(data: any) {
  const sorted = data.Countries.sort(
    (a: any, b: any): any => b.TotalConfirmed - a.TotalConfirmed,
  );
  sorted.forEach((value: any) => {
    const li = document.createElement('li');
